...
test_that("miss_var_run doesn't return a warning", {
  expect_silent(miss_var_run(data = pedestrian, var = hourly_counts))
  expect_silent(miss_var_run(data = p_group, var = hourly_counts))
})

mvr <- miss_var_run(data = pedestrian, var = hourly_counts)

mvr_g <- miss_var_run(data = p_group, var = hourly_counts)
