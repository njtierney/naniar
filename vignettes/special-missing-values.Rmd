...
Data can have special missing values to indicate specific reasons for missingness.
For example, "9999" is sometimes used in weather data, say for for example, the [Global Historical Climate Network (GHCN) data](https://www.ncdc.noaa.gov/ghcn-daily-description)), to indicate specific types of missingness, such as instrument failure.

You might be interested in creating your own special missing values so that you can mark an individual dropping out of a study, for known instrument failure in weather instruments, or for when values are censored in analysis.
...
This reads as "recode shadow for wind where wind is equal to -99, and give it the label "broken_machine". The `.where` function is used to help make our intent clearer, and reads very much like the `dplyr::case_when()` functions.
