#' Return a vector of the number of missing values in each row, or each column
#'
#' Substitute for `rowSums(is.na(data))` or `colSums(is.na(data))` but it also checks if input is NULL or
#'   is a dataframe
#'
...
#' @export
#' @rdname n-miss-row-col
n_miss_col <- function(data){
  test_if_null(data)
  test_if_dataframe(data)
  as.intefer(colSums(is.na(data)))
}
