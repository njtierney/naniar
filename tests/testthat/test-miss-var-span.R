...
mvs <- miss_var_span(data = pedestrian, var = hourly_counts, span_every = 168)

mvs_g <- miss_var_span(data = p_group, var = hourly_counts, span_every = 168)
...
test_that("miss_var_span n_miss + n_complete returns the number of rows", {
  test <- miss_var_span(pedestrian, var = hourly_counts, span_every = 4000)
