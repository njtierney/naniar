...
test_that("impute_below_at leaves no NA values", {
  expect_false(
    anyNA(impute_below_at(miss_df, "miss_vec")[
      which(is.na(miss_df$miss_vec)),
    ])
  )
})
...
test_that("impute_below_at works with vars", {
  expect_false(
    anyNA(impute_below_at(miss_df, dplyr::vars(miss_vec))[
      which(is.na(miss_df$miss_vec)),
    ])
  )
})
...
test_that("impute_below_if leaves no NA values", {
  expect_false(
    anyNA(impute_below_if(airquality, .predicate = is.numeric))
  )
})
