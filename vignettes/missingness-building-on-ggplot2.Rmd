...
Another method of visualisation can be explored using `geom_missing_point()` from the `narnia` package:
...
We can also look at the number and percent of missings in each case and variable with `miss_case_summary`, and `miss_var_summary`.
...
Tabulations of the number of missings in each case or variable can be calculated with `miss_case_table` and `miss_var_table`. 
