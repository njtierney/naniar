...
test_that("only specified columns are affected",{
  df <- dat_ms
  out <- replace_to_na(df, list(x = c(-99)))
...
test_that("works for multiple columns",{
  df <- dat_ms
  out <- replace_to_na(df, list(x = c(-99, -98),
                                y = c("N/A"),
                                z = c(-1, -99)))
