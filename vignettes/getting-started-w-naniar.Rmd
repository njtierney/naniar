...
Another approach to visualising the missings in a dataset is to use the `gg_na_var` plot:
...
This creates a `ggplot2` object with the number of missing values for each variable, ordered from most missing to least missing. Under the hood, this is powered using `na_var_summary()`, described later.

The plots created with the `gg_na` family all have a basic theme, but you can customise them, just like usual ggplot objects. If you call any ggplot customisation functions with a `gg_na` object, the default args will be overridden. 

```{r}
gg_na_var(airquality) + theme_bw() 
...
`naniar` also provide numerical summaries for missing data. Two convenient counters of complete values and missings are `n_na` and `n_complete`. These work on both dataframes and vectors, similar to `dplyr::n_distinct`
...
The syntax for the other numerical sumamries in `naniar` are `na_`, and then `case`, or `var` to refer to cases or variables. There are then `summary`, `table`, `run`, `span`, and `cumsum` options to explore missing data.

The

`na_case_prop` and `na_case_pct` return numeric value describing the proportion or percent of missing values in the dataframe.
...
`na_case_summary` returns a numeric value that describes the number of missings in a given case (aka row), the percent of missings in that row.
...
`na_case_table` tabulates the number of missing values in a case / row. Below, this shows the number of missings in a case:
...
Similar to `na_case_pct()` and `na_case_prop()`, `na_var_pct()` and `na_var_prop()` returns the percent and proportion of variables that contain a missing value.

```{r}

na_var_prop(airquality)
na_var_pct(airquality)

```

`na_var_summary` then returns the number of missing values in a variable, and the percent missing in that variable.
...
Finally, `na_var_table`. This describes the number of missings in a variable.
...
`na_var_run` can be particularly useful in time series data, as it allows you to provide summaries for the number of missings or complete values in a single run. The function `na_var_run()` provides a data.frame of the run length of missings and complete values. To explore this function we will use the built-in dataset, `pedestrian`, which contains hourly counts of pedestrians from four locations around Melbourne, Australia, from 2016.

To use `na_var_run`, you specify the variable that you want to explore the runs of missingness for, in this case, `hourly_counts`:

```{r}
na_var_run(pedestrian,
             hourly_counts)
```

`na_var_span()` is used to determine the number of missings over a specified repeating span of rows in variable of a dataframe. Similar to `na_var_run()`, you specify the variable that you wish to explore, you then also specify the size of the span with the `span_every` argument.

```{r}

na_var_span(pedestrian,
              hourly_counts,
              span_every = 100)
...
Every `na_*` summary function that returns a dataframe can be used with dplyr `group_by`. For example, if you would like to look at the number of missing values for all variables of pedestrian data.

```{r}

pedestrian %>% na_var_summary()
...
Here, the approach is to predict the proportion of missingness in a given case, using all variables. There is a little helper function to add a column with the proportion of cases or rows missing - `add_prop_na`. This created a column named "prop_na", which is the proportion of missing values in that row.
...
Other plans to extend the `geom_na_` family to include:
...
Naming credit (once again!) goes to Miles McBain. Among various other things, Miles also worked out how to overload the missing data and make it work as a geom. Thanks also to Colin Fay for helping me understand tidy evaluation and for features such as `replace_to_na`, `na_*_cumsum`, and more.
