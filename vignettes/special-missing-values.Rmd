...
You might be interested in creating your own special missing values so that you can mark specific, known reasons for missingness. For example, an individual dropping out of a study, known instrument failure in weather instruments, or for values being censored in analysis.
This means that although the value was not recorded when it should have been - it is a missing value - we happen to have some information about _why_ it is missing, which now gets lost if it is recoded as `NA`.  Other stats programming languages like STATA, SAS, and SPSS have this capacity, but currently `R` does not. So, it is
difficult to create these special missing values.

To recode missingness by recoding the missing value `NA` as something else, like `NA_reason` we can use `recode_shadow`. `naniar` makes it easy recode missing values into special missings, by recording them in the `shadow` part of the `nabular` data, which is a special dataframe that contains missingness information. 

This vignette will walk you through how to add special missing values using the
`recode_shadow()` function, and describe how it would work in a workflow. First we consider some terminology to explain these ideas, if you are not familiar with the workflows in `naniar`.
...
All additional types of missingness are recorded across all shadow variables, even if those variables don't contain that special missing value. This ensures all flaviours of missingness are known.

`recode_shadow` provides a flexible and fluent interface to recode, or flag, the shadow information in the shadow matrix as a special type of missing value.

So summarise, to use `recode_shadow`, the user provides the following information:
...
Some examples of using `recode_shadow` in a workflow are now discussed.

## Using `recode_shadow` in a workflow

Now that we have discussed how `recode_shadow` works, we are going to cover using it in a workflow, specifically covering:

* Using `recode_shadow()` with actual data
* Replacing the previous actual values using `replace_with_na()`
* Explore missings where special cases are considered
* Explore imputed values, looking at these special cases

### Using `recode_shadow()` with actual data

To replace the actual data, we need to consider a new dataset, ...


### Replacing the previous actual values using `replace_with_na()`

Once the values are appropriately recorded as special missing values, you might want to change these back to missing values.
